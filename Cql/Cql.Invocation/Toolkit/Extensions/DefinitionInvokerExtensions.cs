        ...
        SelectResultsOptions? selectResultsOptions = null)
    {
        ...
        return definitionInvokers
            .TrySelect(
                ...
                {
                    object?[] args = [];
                    if (definitionInvoker.Operands.Length > 0)
                    {
                        ...
                        if (args.Length != definitionInvoker.Operands.Length)
                            throw new ArgumentException($"The number of arguments ({args.Length}) does not match the number of parameters ({definitionInvoker.Operands.Length}) for definition {definitionInvoker}.");
                    }
