...
namespace Ncqa.Cql.Runtime
{
    public partial class CqlOperatorsBase
    {
        ...
        {
            var result = Context.Compare(left!, right!, precision);
            if (result == null)
                return null;
            else if (result > 0)
                return true;
        ...
        #region  Now
        public CqlDateTime Now()
        {
            var now = Context.Now;
            return new CqlDateTime(now.Year, now.Month, now.Day, now.Hour, now.Minute, now.Second, now.Millisecond, now.Offset.Hours, now.Offset.Minutes);
        }
        #endregion
        ...
        public CqlTime TimeOfDay() => CqlTime.TimeOfDay(Context.Now);

        #endregion

        #region  Today

        public CqlDate Today() => CqlDate.Today(Context.Now);
