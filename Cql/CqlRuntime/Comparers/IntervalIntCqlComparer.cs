...
{
    public class IntervalIntCqlComparer : IntervalCqlComparer
    {
        public IntervalIntCqlComparer(CqlComparers comparers, ICqlOperators operators) : base(comparers, operators)
        {
        }
        ...
        public override int? Compare(object x, object y, string? precision = null)
        {
            if (x == null || y == null)
                return null;
            var xInt = x as CqlInterval<int?> ?? throw new ArgumentException("Wrong type", nameof(x));
            var yInt = y as CqlInterval<int?> ?? throw new ArgumentException("Wrong type", nameof(y));
