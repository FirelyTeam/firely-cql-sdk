...
{
    public class IntervalCqlDateCqlComparer : IntervalCqlComparer
    {
        public IntervalCqlDateCqlComparer(CqlComparers comparers, ICqlOperators operators) : base(comparers, operators)
        {
        }
        ...
        public override int? Compare(object x, object y, string? precision = null)
        {
            if (x == null || y == null)
                return null;
            var xInt = x as CqlInterval<CqlDate> ?? throw new ArgumentException("Wrong type", nameof(x));
            var yInt = y as CqlInterval<CqlDate> ?? throw new ArgumentException("Wrong type", nameof(y));
