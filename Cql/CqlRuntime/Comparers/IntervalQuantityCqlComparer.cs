...
{
    public class IntervalQuantityCqlComparer : IntervalCqlComparer
    {
        public IntervalQuantityCqlComparer(CqlComparers comparers, ICqlOperators operators) : base(comparers, operators)
        {
        }
        ...
        public override int? Compare(object x, object y, string? precision = null)
        {
            if (x == null || y == null)
                return null;
            var xInt = x as CqlInterval<CqlQuantity?> ?? throw new ArgumentException("Wrong type", nameof(x));
            var yInt = y as CqlInterval<CqlQuantity?> ?? throw new ArgumentException("Wrong type", nameof(y));
