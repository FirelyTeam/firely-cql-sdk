    ...
    public partial class CqlOperatorsBase
    {
        ...
        public bool? ListContains<T>(IEnumerable<T> list, T item)
        {
            ...
            foreach (var i in list.Cast<object>())
                if (Context.Compare(item, i, null) == 0)
                    return true;
            return false;
        }
        #endregion
        ...
        public IEnumerable<T>? ListDistinct<T>(IEnumerable<T> source)
        {
            ...
            foreach (object? item in source)
            {
                ...
                else if (!Enumerable.Contains(result!, item!, Context.IEqualityComparer!))
                {
                    result.Add(item!);
                }
            }
        ...
        public bool? ListEqual<T>(IEnumerable<T>? left, IEnumerable<T>? right)
        {
            ...
            while (lit.MoveNext())
            {
                ...
                {
                    onlyNull = false;
                    if (Context.Compare(lv!, rv!, null) != 0)
                        return false;
                }
            }
        ...
        public bool? ListEquivalent<T>(IEnumerable<T>? left, IEnumerable<T>? right)
        {
            ...
            while (lit.MoveNext())
            {
                ...
                else if (Context.Equivalent(lv!, rv!, null) == false)
                    return false;
            }
            if (rit.MoveNext()) // the 2nd list is longer than the 1st.
                return false;
        ...
        public IEnumerable<T>? ListExcept<T>(IEnumerable<T>? left, IEnumerable<T>? right)
        {
            ...
            else
            {
                var except = left
                    .Cast<object>()
                    .Except(right.Cast<object>(), Context.IEqualityComparer)
                    .Cast<T>()
                    .ToList();
                return except;
            }
        ...
        public bool? InList<T>(T element, IEnumerable<T> argument)
        {
            ...
            {
                return argument.Any(t => Context.Compare(element, t!, null) == 0);
            }
        ...
        public bool? CodeInList(CqlCode? element, IEnumerable<CqlCode>? argument)
        {
            ...
            {
                var result = argument.Any(t => Context.Compare(element, t!, null) == 0);
                return result;
            }
        }
        ...
        public bool? ListIncludesList<T>(IEnumerable<T>? left, IEnumerable<T>? right)
        {
            ...
            foreach (var element in right)
            {
                ...
                {
                    if (Context.IEqualityComparer.Equals(element!, t!))
                    {
                        found = true;
                        break;
                    }
        ...
        public int? ListIndex<T>(IEnumerable<T>? list, T element)
        {
            ...
            foreach (T t in list)
            {
                if (Context.IEqualityComparer.Equals(t!, element))
                    return i;
                i++;
            }
            return -1;
        ...
        public IEnumerable<T>? Slice<T>(IEnumerable<T>? source, int? startIndex, int? endIndex)
        {
            ...
            var si = startIndex!.Value;
            if (source is List<T> list)
            {
                var slice = list.GetRange(si, endIndex!.Value < list.Count ? endIndex.Value : (list.Count - 1));
                return slice;
            }
            else
            {
        ...
        public IEnumerable<T>? ListUnion<T>(IEnumerable<T>? left, IEnumerable<T>? right)
        {
            ...
            else
            {
                var union = left
                    .Cast<object>()
                    .Union(right.Cast<object>(), Context.IEqualityComparer)
                    .Cast<T>()
                    .ToList();
                return union;
            }
        ...
        public IEnumerable<CqlCode>? ValueSetUnion(IEnumerable<CqlCode>? left, IEnumerable<CqlCode>? right)
        {
            ...
            else if (left is ValueSetFacade leftFacade)
            {
                ...
                {
                    var union = new ValueSetUnion(new[] { leftFacade, rightFacade }, Context);
                    return union;
                }
                else if (right is ValueSetUnion rightUnion)
                {
                    var all = rightUnion.Facades
                        ...
                        .ToArray();
                    var union = new ValueSetUnion(all, Context);
                    return union;
                }
            }
        ...
        public IEnumerable<T>? ListSortBy<T>(IEnumerable<T>? source, Func<T, object> sortByExpr, SortOrder order)
        {
            ...
            if (order == SortOrder.Ascending)
            {
                var lookup = source.Select(x => new { value = x, key = sortByExpr(x) });

                var ordered = lookup.Select(x => x.key)
                    .OrderBy(t => t, Context.IComparer)
                    .Select(o => lookup.First(l => Context.IEqualityComparer.Equals(l.key, o)).value)
                    .ToList();
            ...
            else if (order == SortOrder.Descending)
            {
                ...
                var ordered = lookup.Select(x => x.key)
                    .OrderByDescending(t => t, Context.IComparer)
                    .Select(o => lookup.First(l => Context.IEqualityComparer.Equals(l.key, o)).value)
                    .ToList();
                return ordered;
            }
