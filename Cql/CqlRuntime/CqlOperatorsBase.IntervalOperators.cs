...
namespace Ncqa.Cql.Runtime
{
    public partial class CqlOperatorsBase
    {
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            var after = Context.Compare(leftClosed!.low!, rightClosed!.high!, precision);
            return after > 0;
        }
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            var after = Context.Compare(closed!.low!, right, precision);
            return after > 0;
        }
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            var after = Context.Compare(left, closed!.high!, precision);
            return after > 0;
        }
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            var before = Context.Compare(leftClosed!.high!, rightClosed!.low!, precision);
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            var before = Context.Compare(closed!.high!, right, precision);
            return before < 0;
        }
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            var before = Context.Compare(left, closed!.low!, precision);
            return before < 0;
        }
        ...
        {
            var ignoreClosed = Context.Options.Standard == RuntimeStandard.ReferenceEngine;
            ...
            var queue = intervals
                ...
                .OrderBy(interval => interval!.low!, Context.IComparer)
                .ToList();
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            var lowCompare = Context.Compare(point, closed!.low!, precision);
            var highCompare = Context.Compare(point, closed!.high!, precision);
        ...
        public bool? Ends<T>(CqlInterval<T> left, CqlInterval<T> right, string? precision)
        {
            ...
            if (Context.Compare(left!.low!, right!.low!, precision) >= 0 && Context.Compare(left.high!, right.high!, precision) == 0)
                return true;
            else
                return false;
        }
        ...
        public IEnumerable<CqlDate>? ExpandInterval(CqlInterval<CqlDate>? argument, CqlQuantity? per)
        {
            ...
            while (Context.Compare(listItem!, highInterval!, null) <= 0);
        ...
        public IEnumerable<decimal?>? ExpandInterval(CqlInterval<decimal?>? argument, CqlQuantity? per)
        {
            ...
            while (Context.Compare(listItem, interval.high!, null) <= 0);
        ...
        public bool? ElementInInterval<T>(T t, CqlInterval<T>? interval, string? precision)
        {
            ...
            var lowCompare = Context.Compare(t, interval.low ?? Minimum<T>()!, precision) ?? (lowClosed ? 0 : null);
            var highCompare = Context.Compare(t, interval.high ?? Maximum<T>()!, precision) ?? (highClosed ? 0 : null);
        ...
        public bool? IntervalIncludesInterval<T>(CqlInterval<T>? larger, CqlInterval<T>? smaller, string precision)
        {
            ...
            var lowCompare = Context.Compare(larger.low ?? Minimum<T>()!, smaller.low ?? Minimum<T>()!, precision);
            var highCompare = Context.Compare(larger.high ?? Maximum<T>()!, smaller.high ?? Maximum<T>()!, precision);
        ...
        public CqlInterval<T>? IntervalIntersectsInterval<T>(CqlInterval<T>? left, CqlInterval<T>? right)
        {
            ...
            if (Context.Compare(leftLow!, rightHigh!, null) > 0 || Context.Compare(rightLow!, leftHigh!, null) > 0) return null;
            else
            {
                T LowValue;
                bool LowValueClosed;
                ...
                var leftCompare = Context.Compare(leftLow!, rightLow!, null);
                if (leftCompare > 0)
                {
                    LowValue = left.low;
                ...
                var rightCompare = Context.Compare(leftHigh!, rightHigh!, null);
                if (rightCompare < 0)
                {
                    HighValue = left.high;
        ...
        public bool? Meets(CqlInterval<int?>? left, CqlInterval<int?>? right, string? precision)
        {
            ...
            return Context.Options.Standard == RuntimeStandard.ReferenceEngine ?
                MeetsIgnoringClosed(left, right, precision, Predecessor) :
                MeetsHelper(ToClosed(left)!, ToClosed(right)!, precision, Predecessor);
        }
        ...
        private bool? MeetsHelper<T>(CqlInterval<T> left, CqlInterval<T> right, string? precision, Func<T, T> predecessor)
        {
            if (left == null || right == null)
                return null;
            if (Context.Compare(left.high!, right.low!, precision) == 0 || Context.Compare(left.low!, right.high!, precision) == 0)
                return true;

            if ((right.lowClosed ?? false) && (left.highClosed ?? false) && Context.Compare(left.high!, predecessor(right.low)!, precision) == 0)
                return true;
            if ((left.lowClosed ?? false) && (right.highClosed ?? false) && Context.Compare(predecessor(left.low)!, right.high!, precision) == 0)
                return true;

            return false;
        }
        private bool? MeetsIgnoringClosed<T>(CqlInterval<T> left, CqlInterval<T> right, string? precision, Func<T, T> predecessor)
        {
            if (left == null || right == null)
                return null;
            if (Context.Compare(left.high!, right.low!, precision) == 0 || Context.Compare(left.low!, right.high!, precision) == 0)
            ...
            if (Context.Compare(left.high!, predecessor(right.low)!, precision) == 0)
                return true;
            if (Context.Compare(predecessor(left.low)!, right.high!, precision) == 0)
                return true;
        ...
        public bool? MeetsAfter(CqlInterval<int?>? left, CqlInterval<int?> right, string? precision)
        {
            ...
            return Context.Options.Standard == RuntimeStandard.ReferenceEngine ?
                MeetsAfterIgnoringClosed(left, right, precision, Predecessor) :
                MeetsAfterHelper(ToClosed(left), ToClosed(right), precision, Predecessor);
        }
        ...
        private bool? MeetsAfterHelper<T>(CqlInterval<T>? left, CqlInterval<T>? right, string? precision, Func<T, T> predecessor)
        {
            if (left == null || right == null)
                return null;
            if (Context.Compare(left.low!, right.high!, precision) == 0)
                return true;
        ...
        public bool? MeetsBefore(CqlInterval<int?> left, CqlInterval<int?> right, string? precision)
        {
            ...
            return Context.Options.Standard == RuntimeStandard.ReferenceEngine ?
                MeetsBeforeIgnoringClosed(left, right, precision, Predecessor) :
                MeetsBeforeHelper(ToClosed(left)!, ToClosed(right)!, precision, Predecessor);
        }
        ...
        private bool? MeetsBeforeIgnoringClosed<T>(CqlInterval<T> left, CqlInterval<T> right, string? precision, Func<T, T> predecessor)
        {
            if (left == null || right == null)
                return null;
            if (Context.Compare(left.high!, right.low!, precision) == 0)
                return true;
        ...
        public bool? IntervalSameAs<T>(CqlInterval<T> @this, CqlInterval<T> other, string? precision)
        {
            if (@this == null || other == null)
                ...
                return Context.Compare(@this, other, precision) == 0;
        }
        ...
        public bool? IntervalSameOrAfter(CqlInterval<CqlDate?>? @this, CqlInterval<CqlDate?>? other, string? precision)
        {
            ...
            var isSame = Context.Compare(thisClosed, otherClosed, precision) == 0;
            if (isSame)
                return true;

            var boundaryHit = Context.Compare(thisClosed.low!, otherClosed.high!, precision) == 0;
            if (boundaryHit)
                return true;
        ...
        public bool? IntervalSameOrBefore(CqlInterval<CqlDate?>? @this, CqlInterval<CqlDate?>? other, string? precision)
        {
            ...
            var boundaryHit = Context.Compare(thisClosed.high!, otherClosed.low!, precision) == 0;
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?>? toClosed)
        {
            ...
            if (Context.Compare(left.high ?? Maximum<T>()!, right.low ?? Minimum<T>()!, precision) >= 0
                && Context.Compare(left.low ?? Minimum<T>()!, right.high ?? Maximum<T>()!, precision) <= 0)
                return true;
            else
                return false;
        }
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            left = toClosed(left!)!;
            right = toClosed(right!)!;
            if (Context.Compare(left.low ?? Maximum<T>()!, right.high ?? Minimum<T>()!, precision) <= 0
                && Context.Compare(left.high ?? Minimum<T>()!, right.high ?? Maximum<T>()!, precision) > 0)
                return true;
            else
                return false;
        }
            ...
            Func<CqlInterval<T?>?, CqlInterval<T?>?> toClosed)
        {
            ...
            if (Context.Compare(left!.high ?? Maximum<T>()!, right!.low ?? Minimum<T>()!, precision) >= 0
                && Context.Compare(left.low ?? Minimum<T>()!, right.low ?? Minimum<T>()!, precision) < 0)
                return true;
            else
                return false;
        }
        ...
        public T? PointFrom<T>(CqlInterval<T?>? argument) =>
           argument == null ? default : (Context.Compare(argument!.low!, argument!.high!, null) == 0 ? argument.low : throw new InvalidOperationException("PointFrom can not be extracted  - interval is too wide"));
        ...
        public bool? IntervalProperlyIncludedInInterval<T>(CqlInterval<T>? left, CqlInterval<T>? right, string precision)
        {
            ...
            var low = Context.Compare(left!.low ?? min!, right.low ?? min, precision);
            if (low < 0)
                return false;
            var max = Maximum<T>()!;
            var high = Context.Compare(left.high ?? max, right.high ?? max, precision);
            if (high > 0)
                return false;
        ...
        public bool? ElementProperlyIncludedInInterval<T>(T left, CqlInterval<T>? right)
        {
            ...
            var low = Context.Compare(left, right.low, null);
            var high = Context.Compare(left, right.high, null);
            if (low < 0)
                return false;
            if (high > 0)
                return false;
        ...
        public bool? ElementProperlyIncludedInInterval(CqlDate left, CqlInterval<CqlDate>? right, string precision)
        {
            ...
            var low = Context.Compare(left, right.low, precision);
            var high = Context.Compare(left, right.high, precision);
            if (low < 0)
                return false;
            if (high > 0)
                return false;
        ...
        public bool? Starts<T>(CqlInterval<T>? starts, CqlInterval<T>? other, string? precision)
        {
            if (starts == null || other == null)
                return null;
            if (Context.Compare(starts.low!, other.low!, precision) == 0 && Context.Compare(starts.high!, other.high!, precision) <= 0)
                return true;
            return false;
        }
        ...
        public CqlInterval<T>? IntervalUnion<T>(CqlInterval<T>? left, CqlInterval<T>? right)
        {
            ...
            {
                if (Context.Compare(left.low!, right.low!, null) <= 0)
                {
                    if (Context.Compare(left.high!, right.low!, null) >= 0)
                    {
                        if (Context.Compare(left.high!, right.high!, null) < 0) return new CqlInterval<T>(left.low, right.high, left.lowClosed, right.highClosed);
                        else return left;
                    }
                    else return null;
                }
                ...
                {
                    if (Context.Compare(right.high!, left.low!, null) >= 0)
                    {
                        if (Context.Compare(left.high!, left.high!, null) > 0) return new CqlInterval<T>(right.low, left.high, right.lowClosed, left.highClosed);
                        else return right;
                    }
                    else return null;
                }
