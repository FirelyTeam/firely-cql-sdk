...
public class TagsTest : Base
{
    ...
    public void Tags_Containing_Quotes_Must_Be_Escaped_In_CSharp()
    {
        var cqlLibraryString = CqlLibraryString.Parse(
            ...
            """);
        var cqlToolkit = CreateFluentCqlToolkit().AddCqlLibraryString(cqlLibraryString).ProcessCqlToElm();

        var elmToolkit = cqlToolkit.CreateFluentElmToolkit().ProcessElmToAssemblies();

        _ = elmToolkit.ElmToAssemblyConversions.First().Value.CSharpSourceCode; // Debug this line to see the C# code generated by the CQL-to-ELM compiler

        using var librarySetInvoker = elmToolkit.CreateLibrarySetInvoker();
        _ = librarySetInvoker.LibraryInvokers[cqlLibraryString.VersionedLibraryIdentifier]
