...
namespace Hl7.Cql.CqlToElm.Test
{
    ...
    public class NotEqualTest : Base
    {
        ...
        {
            var lib = CreateFluentCqlToolkit().MakeLibraryFromExpression("{ x: 1, y: null } = { x: 1, y: 2 }");
        ...
        {
            var lib = CreateFluentCqlToolkit().MakeLibraryFromExpression("{ x: 1, y: null } = { x: 1, y: null }");
            var equal = lib.Should().BeACorrectlyInitializedLibraryWithStatementOfType<Equal>();
            var eq = Run<bool?>(equal, lib);
            eq.Should().BeTrue();
        }
