...
namespace Hl7.Cql.CqlToElm.Test
{
    ...
    public class InTest : Base
    {
        ...
        {
            // The C# CQL-to-ELM compiler cannot resolve the signature for "null contains 5"
            // because null has type Any which doesn't match IEnumerable<T> or CqlInterval<T>.
            // However, the ExpressionBuilder should not throw an exception when processing
            // such ELM (which might have been generated by the Java CQL-to-ELM compiler).
            var lib = CreateCqlToolkit().MakeLibraryFromExpression("null contains 5", 
                expectedErrors: ["Could not resolve call to operator Contains with signature (Any, Integer)."]);
            // The library should have a Contains expression even though it has a compilation error
            Assert.IsNotNull(lib.statements);
            Assert.AreEqual(1, lib.statements.Length);
            Assert.IsInstanceOfType(lib.statements[0].expression, typeof(Contains));
        }
