...
namespace Hl7.Cql.CqlToElm.Test
{
    ...
    public class ComponentFromTest : Base
    {
        ...
        {
            var lib = CreateFluentCqlToolkit().MakeLibraryFromExpression("year from DateTime(2003, 10, 29, 20, 50, 33, 955)");
        ...
        {
            var lib = CreateFluentCqlToolkit().MakeLibraryFromExpression("year from Date(2003, 10, 29)");
            var cf = lib.Should().BeACorrectlyInitializedLibraryWithStatementOfType<DateTimeComponentFrom>();
            var result = Run<int?>(cf, lib);
            Assert.AreEqual(2003, result);
        }
        ...
        {
            var lib = CreateFluentCqlToolkit().MakeLibraryFromExpression("hour from Time(20, 40, 20, 123)");
            var cf = lib.Should().BeACorrectlyInitializedLibraryWithStatementOfType<DateTimeComponentFrom>();
            var result = Run<int?>(cf, lib);
        ...
        {
            var lib = CreateFluentCqlToolkit().MakeLibraryFromExpression("timezoneoffset from DateTime(2003, 10, 29, 20, 50, 33, 955, 5.5)");
        ...
        {
            var lib = CreateFluentCqlToolkit().MakeLibraryFromExpression("date from DateTime(2003, 10, 29, 20, 50, 33, 955, 5.5)");
