...
namespace Hl7.Cql.Compiler
{
    internal partial class ExpressionBuilder
    {
        ...
        protected Expression NotEqual(elm.NotEqual eq, ExpressionBuilderContext ctx)
        {
            ...
            var not = OperatorBinding.Bind(CqlOperator.Not, ctx.CqlContextParameter, equal);
            return not;
        }
        ...
        protected Expression Equal(Expression left, Expression right, ExpressionBuilderContext ctx)
        {
            ...
            else if (IsOrImplementsIEnumerableOfT(left.Type))
            {
                ...
                if (IsOrImplementsIEnumerableOfT(right.Type))
                {
                    ...
                    var call = OperatorBinding.Bind(CqlOperator.ListEqual, ctx.CqlContextParameter, left, right);
                    return call;
                }
            ...
            {
                var call = OperatorBinding.Bind(CqlOperator.Equal, ctx.CqlContextParameter, left, right);
                return call;
            }
        }
        ...
        protected Expression Equivalent(elm.Equivalent eqv, ExpressionBuilderContext ctx)
        {
            ...
            if (IsOrImplementsIEnumerableOfT(left.Type))
            {
                ...
                if (IsOrImplementsIEnumerableOfT(right.Type))
                {
                    ...
                    var call = OperatorBinding.Bind(CqlOperator.ListEquivalent, ctx.CqlContextParameter, left, right);
                    return call;
                }
                else
                {
            ...
            {
                var call = OperatorBinding.Bind(CqlOperator.Equivalent, ctx.CqlContextParameter, left, right);
                return call;
            }
        }
