...
- Added `Hl7.Cql.Fhir.FhirTypeConverter.ConvertRangeToCqlInterval(this TypeConverter, Range?, string?)`, which picks the
  CQL interval type for a FHIR `Range` from a CQL type name (typically the full `cqf-cqlType` extension value from a
  Library parameter, such as `Interval<Integer>` or `Interval<Long>`): wrapped and bare `Integer`, `Decimal` and
  `Long` hints yield an interval of that type, anything else - including an absent hint - the widest reading, an
  `Interval<Quantity>`. It is the `Range` counterpart of `ConvertPeriodToCqlInterval`, and both helpers now recognize
  the wrapped `Interval<...>` format emitted by the SDK packager. (#1454)
