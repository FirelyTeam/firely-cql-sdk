...
- Quantity equality (`=`) and the ordering comparisons no longer treat quantities of different dimensions as comparable. Each operand was canonicalized to its UCUM base unit and only the resulting *values* were compared, so `1 'cm' = 0.01 'g'` returned `true` and `1 'cm' < 1 'g'` answered as if both were dimensionless. The canonical units must now agree; incommensurable units take the same unsupported-comparison path (a `NotSupportedException`) that units which cannot be canonicalized at all already took. Comparisons between convertible units (`1 'cm' = 0.01 'm'`) are unchanged. Equivalence (`~`) is unaffected and still never signals an error — including for an interval over incommensurable quantity bounds, which answers `false` where it previously reported the two intervals equivalent (#1417).
