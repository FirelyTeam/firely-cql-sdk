...
- `VersionedIdentifier.GetHashCode()` is now consistent with `Equals()` by hashing library ids case-insensitively and not hashing raw version text, preventing missed matches/duplicate entries in hash-based dependency sets when identifiers differ only by case or semantically-equivalent version formatting (#1375).
