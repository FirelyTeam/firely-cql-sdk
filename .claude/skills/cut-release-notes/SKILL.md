...
1. **Decide the version level before writing anything.** Walk **every** trigger in [docs/versioning.md](../../../docs/versioning.md) against the actual release window — that list is canonical and is deliberately not copied here, because a copy drifts and then under-calls a level. Record the level and the trigger that forced it; this becomes the release-notes document's `### Version Level` section and goes in the version-bump PR's description. Do not justify the level by what previous releases did; the history is inconsistent (`2.9.1` shipped a breaking change as a patch), so precedent proves nothing.
