...
define "Injury due to falling rock within measurement period":   
    [Condition: "Injury due to falling rock"] C
       where (C.onset as dateTime) during "Measurement Period"
          
define "Latest injury due to falling rock":
      Last( "Injury due to falling rock within measurement period" C sort by (onset as dateTime).value )
/*
...
define "Tiny Umbrella Supply within 7 days after most recent injury due to falling rock":
    [SupplyDelivery] SD 
      where (SD.suppliedItem.item ~ "Tiny Umbrella")
       and (("Latest injury due to falling rock" C where (C.onset as dateTime) 7 days or less before (SD.occurrence as dateTime)) is not null)
