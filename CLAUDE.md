...
- **Never put backstory in a code comment.** Why a thing was once done, which PR or issue caused it, what used to be pinned/broken/worked-around — none of that belongs in source, project or config files. It goes in the **PR description** (and the release notes where consumers need it). A comment may state a constraint that is true *now* ("raise this property rather than pinning one package"), never the history of how it came to be. History rots in place, gets copied forward, and outlives the reason it was written; git blame, the PR and the issue already hold it.
...
The `*.g.cs` files are not the only generated artifact carrying that version. `Demo/Measures.dqm-content-qicore-2025` also vendors library resources into the `Firely.Cql.Sdk.Integration.Runner` submodule (`IntegrationRunner/Resources/libraries/*.json`), and those JSONs embed the **compiled assembly the integration runner actually executes** — so a bump that skips them leaves the MADiE run exercising the old generator, and the submodule's `passed.jsonl` describing behavior no longer in `develop`. Regenerate them in the same PR with `dotnet build Demo/Measures.dqm-content-qicore-2025/Measures.dqm-content-qicore-2025.csproj -c Release /p:ElmToolingEnabled=true` (build `Cql/PackagerCLI` in the same configuration first — the `GenerateCSharp` target silently skips when the packager exe is missing), then re-run the integration suite, refresh `passed.jsonl` only when the run is complete and has no unexplained regressions, and bump the submodule pointer here. If a complete trustworthy run is blocked, preserve the existing `passed.jsonl` baseline and track the blocker in an issue instead of normalizing coverage loss by replacing the baseline. Unlike the `*.g.cs` header churn, these resources are committed wholesale — the embedded assembly changes in every one of them. Letting this drift is what [#1407](https://github.com/FirelyTeam/firely-cql-sdk/issues/1407) had to clean up.
