...
4.6.3 The version level of the release a fragment lands in is decided by its `##` heading — see [11-versioning.md](11-versioning.md) and [docs/versioning.md](../../docs/versioning.md). A `## Breaking` or `## Potentially Breaking` fragment forces MESO; `## Features`, `## Fixes` and `## Performance` are MICRO by default, but still force MESO when the change moves CQL evaluation results or `GeneratorToolVersion`. Say so in the fragment text when it does.
