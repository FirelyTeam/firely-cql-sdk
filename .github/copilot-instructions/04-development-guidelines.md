...
4.6.3 The version level of the release a fragment lands in is decided by its `##` heading. [docs/versioning.md](../../docs/versioning.md) is canonical for the full trigger list and is deliberately not restated here. Two rules catch people out: a `## Fixes` or `## Performance` fragment still forces **MESO** when the change moves CQL evaluation results or `GeneratorToolVersion` — say so in the fragment text when it does — and public API additions on their own never raise the digit.
