...
public class HybridHWMFHIR_0_102_005
{
    ...
    private CqlValueSet Platelet_count_lab_test_Value() =>
		new CqlValueSet("http://cts.nlm.nih.gov/fhir/ValueSet/2.16.840.1.113762.1.4.1045.127", 
			null);
    ...
    private IEnumerable<Encounter> Inpatient_Encounters_Value()
	{
		...
		null))?.Operators?.TypeConverter).Convert<string>(a_);
			...
			var f_ = MATGlobalCommonFunctionsFHIR4_6_1_000.HospitalizationWithObservation(e_);
			...
			var y_ = context?.Operators.Interval((int?)65, (int?)94, true, true);
			var z_ = context?.Operators.ElementInInterval<int?>(x_, y_, null);

			return context?.Operators.And(q_, z_);
		};
