...
public class HFBetaBlockerTherapyforLVSDFHIR_1_3_000
{
	...
	private bool? Has_Consecutive_Heart_Rates_Less_than_50_Value()
	{
		...
		bool? e_(Tuples.Tuple_FgYDjIJBhiXdBHJjiISjVeOjV tuple_fgydjijbhixdbhjjiisjveojv)
		{
			...
			var p_ = context.Operators.Convert<Code<ObservationStatus>>(tuple_fgydjijbhixdbhjjiisjveojv.HeartRate?.StatusElement?.Value);
			...
			var u_ = FHIRHelpers_4_3_000.ToQuantity(tuple_fgydjijbhixdbhjjiisjveojv.HeartRate?.Value);
			...
			{
				var ai_ = FHIRHelpers_4_3_000.ToInterval(tuple_fgydjijbhixdbhjjiisjveojv.ModerateOrSevereLVSDHFOutpatientEncounter?.Period);
				var aj_ = FHIRHelpers_4_3_000.ToValue(MostRecentPriorHeartRate?.Effective);
				var ak_ = QICoreCommon_2_0_000.toInterval(aj_);
				var al_ = context.Operators.IntervalIncludesInterval<CqlDateTime>(ai_, ak_, null);
				var an_ = QICoreCommon_2_0_000.toInterval(aj_);
				var ao_ = FHIRHelpers_4_3_000.ToValue(tuple_fgydjijbhixdbhjjiisjveojv.HeartRate?.Effective);
				var ap_ = QICoreCommon_2_0_000.toInterval(ao_);
				var aq_ = context.Operators.IntervalBeforeInterval(an_, ap_, null);
			...
			var ae_ = FHIRHelpers_4_3_000.ToQuantity(ad_?.Value);
			var ag_ = context.Operators.Less(ae_, v_);
	...
	private bool? Has_Medical_or_Patient_Reason_for_Not_Ordering_Beta_Blocker_for_LVSD_Value()
	{
		...
		{
			var o_ = FHIRHelpers_4_3_000.ToConcept(NoBetaBlockerOrdered?.Medication);
			var p_ = this.Beta_Blocker_Therapy_for_LVSD();
			var q_ = context.Operators.ConceptInValueSet(o_, p_);
			CqlConcept r_(CodeableConcept @this)
			{
				var ab_ = FHIRHelpers_4_3_000.ToConcept(@this);

				return ab_;
			};
			var s_ = context.Operators.SelectOrNull<CodeableConcept, CqlConcept>(NoBetaBlockerOrdered?.ReasonCode, r_);
			...
			var w_ = context.Operators.SelectOrNull<CodeableConcept, CqlConcept>(NoBetaBlockerOrdered?.ReasonCode, v_);
			var x_ = this.Patient_Reason();
			var y_ = context.Operators.ConceptsInValueSet(w_, x_);
			var z_ = context.Operators.Or(u_, y_);
			var aa_ = context.Operators.And(q_, z_);
