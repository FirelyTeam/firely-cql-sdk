...
;
	...
	private IEnumerable<Encounter> Delivery_Encounters_with_Severe_Obstetric_Complications_Diagnosis_or_Procedure_Excluding_Blood_Transfusion_Value()
	{
		...
		bool? b_(Encounter TwentyWeeksPlusEncounter)
		{
			bool? d_(Encounter.DiagnosisComponent EncounterDiagnoses)
			{
				...
				{
					var z_ = context.Operators.Equal(@this?.Url, "http://hl7.org/fhir/us/qicore/StructureDefinition/qicore-encounter-diagnosisPresentOnAdmission");
				...
				var w_ = this.Present_on_Admission_is_No_or_Unable_To_Determine();
				var x_ = context.Operators.ConceptInValueSet(v_, w_);
				var y_ = context.Operators.And(p_, x_);
			...
			{
				var aa_ = context.Operators.EnumEqualsString(EncounterSMMProcedures?.StatusElement?.Value, "completed");
				var ab_ = FHIRHelpers_4_3_000.ToValue(EncounterSMMProcedures?.Performed);
				var ac_ = QICoreCommon_2_0_000.toInterval(ab_);
				var ad_ = context.Operators.Start(ac_);
				var ae_ = PCMaternal_5_16_000.hospitalizationWithEDOBTriageObservation(TwentyWeeksPlusEncounter);
	...
	public IEnumerable<CqlConcept> pOAIsYesOrExempt(Encounter TheEncounter)
	{
		bool? a_(Encounter.DiagnosisComponent EncounterDiagnoses)
		{
			...
			{
				var m_ = context.Operators.Equal(@this?.Url, "http://hl7.org/fhir/us/qicore/StructureDefinition/qicore-encounter-diagnosisPresentOnAdmission");
			...
			var k_ = this.Present_On_Admission_is_Yes_or_Exempt();
		...
		{
			var n_ = CQMCommon_2_0_000.getCondition(EncounterDiagnoses?.Condition);
			var o_ = FHIRHelpers_4_3_000.ToConcept(n_?.Code);
	...
	private IEnumerable<Tuples.Tuple_CYVVSdgZbMfXHMiBHjISgejQI> Risk_Variable_Heart_Rate_Value()
	{
		...
		Tuples.Tuple_CYVVSdgZbMfXHMiBHjISgejQI b_(Encounter TwentyWeeksPlusEncounter)
		{
			...
			bool? e_(Observation HeartRate)
			{
				...
				var aa_ = context.Operators.Interval(y_, z_, true, false);
				var ab_ = context.Operators.ElementInInterval<CqlDateTime>(u_, aa_, null);
				var ac_ = context.Operators.Convert<Code<ObservationStatus>>(HeartRate?.StatusElement?.Value);
			...
			bool? l_(Observation HeartRate)
			{
				...
				var as_ = context.Operators.Convert<Code<ObservationStatus>>(HeartRate?.StatusElement?.Value);
			...
			var s_ = new Tuples.Tuple_CYVVSdgZbMfXHMiBHjISgejQI
			{
				EncounterId = TwentyWeeksPlusEncounter?.IdElement?.Value,
				FirstHRResult = (j_ as CqlQuantity),
				Timing = r_,
			};
	...
	private IEnumerable<Tuples.Tuple_DBZhWNcHciGGJUSXZKiOPXJYf> Risk_Variable_Systolic_Blood_Pressure_Value()
	{
		...
		Tuples.Tuple_DBZhWNcHciGGJUSXZKiOPXJYf b_(Encounter TwentyWeeksPlusEncounter)
		{
			...
			bool? e_(Observation BP)
			{
				...
				var af_ = context.Operators.Convert<Code<ObservationStatus>>(BP?.StatusElement?.Value);
			...
			{
				var am_ = FHIRHelpers_4_3_000.ToConcept(C?.Code);
				var an_ = this.Systolic_blood_pressure();
				var ao_ = context.Operators.ConvertCodeToConcept(an_);
				var ap_ = context.Operators.Equivalent(am_, ao_);
			...
			{
				var aq_ = FHIRHelpers_4_3_000.ToValue(C?.Value);
			...
			{
				var ar_ = FHIRHelpers_4_3_000.ToValue(BP?.Effective);
				var as_ = QICoreCommon_2_0_000.earliest(ar_);
				var at_ = PCMaternal_5_16_000.hospitalizationWithEDOBTriageObservation(TwentyWeeksPlusEncounter);
				var au_ = context.Operators.Start(at_);
				var av_ = context.Operators.Quantity(1440m, "minutes");
				var aw_ = context.Operators.Subtract(au_, av_);
				var ax_ = PCMaternal_5_16_000.lastTimeOfDelivery(TwentyWeeksPlusEncounter);
				var ay_ = context.Operators.Interval(aw_, ax_, true, false);
				var az_ = context.Operators.ElementInInterval<CqlDateTime>(as_, ay_, null);
				var ba_ = context.Operators.Convert<Code<ObservationStatus>>(BP?.StatusElement?.Value);
	...
	public IEnumerable<CqlConcept> pOAIsNoOrUTD(Encounter TheEncounter)
	{
		bool? a_(Encounter.DiagnosisComponent EncounterDiagnoses)
		{
			...
			var i_ = context.Operators.SingleOrNull<DataType>(h_);
			var j_ = FHIRHelpers_4_3_000.ToConcept(i_);
			var k_ = this.Present_on_Admission_is_No_or_Unable_To_Determine();
			var l_ = context.Operators.ConceptInValueSet(j_, k_);
