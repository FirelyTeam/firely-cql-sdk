...
public class UseofHighRiskMedicationsintheElderlyFHIR_0_1_000
{
	...
	public decimal? MedicationRequestPeriodInDays(MedicationRequest Request)
	{
		...
		{
			var e_ = FHIRHelpers_4_3_000.ToQuantity(R?.DispenseRequest?.ExpectedSupplyDuration);
			...
			var t_ = CumulativeMedicationDuration_4_0_000.Quantity(h_?.Timing?.Repeat?.PeriodElement?.Value, s_);
			var u_ = CumulativeMedicationDuration_4_0_000.ToDaily((h_?.Timing?.Repeat?.FrequencyMaxElement?.Value ?? h_?.Timing?.Repeat?.FrequencyElement?.Value), t_);
			var w_ = context.Operators.LateBoundProperty<IEnumerable<CqlTime>>(h_?.Timing?.Repeat?.TimeOfDayElement, "value");
			var x_ = context.Operators.CountOrNull<CqlTime>(w_);
			var y_ = context.Operators.ConvertIntegerToDecimal(x_);
			var z_ = context.Operators.Multiply((k_ ?? (n_ as CqlQuantity))?.value, ((u_ ?? y_) ?? (decimal?)1.0m));
			var aa_ = context.Operators.Divide(g_?.value, z_);
			var ab_ = context.Operators.Add((int?)1, (R?.DispenseRequest?.NumberOfRepeatsAllowedElement?.Value ?? (int?)0));
			var ac_ = context.Operators.ConvertIntegerToDecimal(ab_);
			var ad_ = context.Operators.Multiply((f_?.value ?? aa_), ac_);
