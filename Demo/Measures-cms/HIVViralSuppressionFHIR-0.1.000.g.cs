...
;
	...
	private Observation Most_Recent_Viral_Load_Test_During_Measurement_Period_Value()
	{
		...
		bool? c_(Observation ViralLoad)
		{
			object h_()
			{
				bool l_()
				{
					object o_ = FHIRHelpers_4_3_000.ToValue(ViralLoad?.Effective);
				...
				bool m_()
				{
					object q_ = FHIRHelpers_4_3_000.ToValue(ViralLoad?.Effective);
				...
				bool n_()
				{
					object s_ = FHIRHelpers_4_3_000.ToValue(ViralLoad?.Effective);
				...
				if (l_())
				{
					object u_ = FHIRHelpers_4_3_000.ToValue(ViralLoad?.Effective);

					return ((u_ as CqlDateTime) as object);
				}
				else if (m_())
				{
					object v_ = FHIRHelpers_4_3_000.ToValue(ViralLoad?.Effective);

					return ((v_ as CqlInterval<CqlDateTime>) as object);
				}
				else if (n_())
				{
					object w_ = FHIRHelpers_4_3_000.ToValue(ViralLoad?.Effective);
		...
		{
			object x_ = FHIRHelpers_4_3_000.ToValue(@this?.Effective);
	...
	private bool? Numerator_Value()
	{
		...
		CqlQuantity c_ = context.Operators.Quantity(200m, "{copies}/mL");
		bool? d_ = context.Operators.Less((b_ as CqlQuantity), c_);
		object f_ = FHIRHelpers_4_3_000.ToValue(a_?.Value);
		CqlCode g_ = this.Below_threshold_level__qualifier_value_();
		CqlConcept h_ = context.Operators.ConvertCodeToConcept(g_);
		bool? i_ = context.Operators.Equivalent((f_ as CqlConcept), h_);
		bool? j_ = context.Operators.Or(d_, i_);
		object l_ = FHIRHelpers_4_3_000.ToValue(a_?.Value);
		CqlCode m_ = this.Not_detected__qualifier_value_();
		CqlConcept n_ = context.Operators.ConvertCodeToConcept(m_);
		bool? o_ = context.Operators.Equivalent((l_ as CqlConcept), n_);
