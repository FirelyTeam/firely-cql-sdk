...
public class AlaraCTFHIR_0_1_001
{
	...
	private IEnumerable<Observation> Qualifying_CTScan_Value()
	{
		...
		bool? d_(Observation CTScanResult)
		{
			...
			object g_ = FHIRHelpers_4_3_000.ToValue(CTScanResult?.Effective);
			...
			CqlDateTime k_ = context.Operators.ConvertStringToDateTime(j_?.BirthDateElement?.Value);
			CqlDateTime m_ = context.Operators.Start(f_);
			int? n_ = context.Operators.CalculateAgeAt(k_, m_, "year");
	...
	private IEnumerable<Observation> Qualifying_CTScan_with_Values_Value()
	{
		...
		bool? b_(Observation CTScan)
		{
			...
			object i_ = FHIRHelpers_4_3_000.ToValue(CTScan?.Value);
	...
	private bool? Denominator_Exclusion_Value()
	{
		...
		bool? b_(Observation CTScan)
		{
			object e_ = FHIRHelpers_4_3_000.ToValue(CTScan?.Value);
			bool? f_(CqlCode @this)
			{
				bool? k_ = context.Operators.Not((bool?)(@this?.code is null));

				return k_;
			};
			IEnumerable<CqlCode> g_ = context.Operators.Where<CqlCode>(((e_ as CqlConcept)?.codes as IEnumerable<CqlCode>), f_);
			string h_(CqlCode @this) => 
				@this?.code;
			IEnumerable<string> i_ = context.Operators.Select<CqlCode, string>(g_, h_);
			bool? j_ = context.Operators.ListContains<string>(i_, "FULLBODY");
