...
public class DiabetesHemoglobinA1cHbA1cPoorControl9FHIR_0_1_000
{
		...
		bool? d_(Observation RecentHbA1c)
		{
			object i_()
			{
				...
				else if (n_())
				{
					...
					CqlInterval<CqlDateTime> ac_ = context.Operators.Convert<CqlInterval<CqlDateTime>>(ab_);

					return ac_ as object;
				}
				else if (o_())
				{
					DataType ad_ = RecentHbA1c?.Effective;
	...
	private bool? Has_Most_Recent_Elevated_HbA1c_Value()
	{
		...
		CqlQuantity d_ = context.Operators.Convert<CqlQuantity>(c_);
	...
	private bool? Has_No_Record_Of_HbA1c_Value()
	{
		...
		bool? d_(Observation NoHbA1c)
		{
			object h_()
			{
				...
				{
					DataType ac_ = NoHbA1c?.Effective;
