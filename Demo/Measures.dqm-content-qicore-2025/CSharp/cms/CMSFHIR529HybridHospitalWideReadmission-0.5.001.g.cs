    ...
    private IEnumerable<(CqlTupleMetadata, string EncounterId, CqlQuantity FirstResult, CqlDateTime Timing)?> SDE_Encounter_With_First_Glucose_Lab_Test_Compute(CqlContext context)
    {
        ...
        (CqlTupleMetadata, string EncounterId, CqlQuantity FirstResult, CqlDateTime Timing)? b_(Encounter EncounterInpatient) {
            ...
            bool? o_(Observation GlucoseLab) {
                ...
                CqlInterval<CqlDateTime> av_ = CQMCommon_4_1_000.Instance.hospitalizationWithObservationAndOutpatientSurgeryService(context, EncounterInpatient);
                bool? aw_ = context.Operators.In<CqlDateTime>(au_, av_, (string)default);
                Code<ObservationStatus> ax_ = GlucoseLab?.StatusElement;
                ObservationStatus? ay_ = ax_?.Value;
                string az_ = context.Operators.Convert<string>(ay_);
                string[] ba_ = [
                    "final",
    ...
    private IEnumerable<(CqlTupleMetadata, string EncounterId, CqlQuantity FirstResult, CqlDateTime Timing)?> SDE_Encounter_With_First_Potassium_Lab_Test_Compute(CqlContext context)
    {
        ...
        (CqlTupleMetadata, string EncounterId, CqlQuantity FirstResult, CqlDateTime Timing)? b_(Encounter EncounterInpatient) {
            ...
            bool? o_(Observation PotassiumLab) {
                ...
                CqlInterval<CqlDateTime> av_ = CQMCommon_4_1_000.Instance.hospitalizationWithObservationAndOutpatientSurgeryService(context, EncounterInpatient);
                bool? aw_ = context.Operators.In<CqlDateTime>(au_, av_, (string)default);
                Code<ObservationStatus> ax_ = PotassiumLab?.StatusElement;
                ObservationStatus? ay_ = ax_?.Value;
                string az_ = context.Operators.Convert<string>(ay_);
                string[] ba_ = [
                    "final",
                ...
                bool? bb_ = context.Operators.In<string>(az_, (IEnumerable<string>)ba_);
                bool? bc_ = context.Operators.And(aw_, bb_);
                DataType bd_ = PotassiumLab?.Value;
                object be_ = FHIRHelpers_4_4_000.Instance.ToValue(context, bd_);
                bool? bf_ = context.Operators.Not((bool?)(be_ is null));
                bool? bg_ = context.Operators.And(bc_, bf_);
                return bg_;
