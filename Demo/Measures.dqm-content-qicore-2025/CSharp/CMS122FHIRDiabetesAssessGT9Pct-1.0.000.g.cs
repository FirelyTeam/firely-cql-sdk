...
public partial class CMS122FHIRDiabetesAssessGT9Pct_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS122FHIRDiabetesAssessGT9Pct_1_0_000>
{
    ...
    public IEnumerable<Observation> Glycemic_Status_Assessment(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Observation>>(
            _cacheIndex_Glycemic_Status_Assessment,
            Glycemic_Status_Assessment_Compute);
    ...
    public CqlDate Most_Recent_Glycemic_Status_Date(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<CqlDate>(
            _cacheIndex_Most_Recent_Glycemic_Status_Date,
            Most_Recent_Glycemic_Status_Date_Compute);
    ...
    public Observation Lowest_Glycemic_Status_Assessment_Reading_on_Most_Recent_Day(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<Observation>(
            _cacheIndex_Lowest_Glycemic_Status_Assessment_Reading_on_Most_Recent_Day,
            Lowest_Glycemic_Status_Assessment_Reading_on_Most_Recent_Day_Compute);
    ...
    public bool? Has_Most_Recent_Glycemic_Status_Assessment_Without_Result(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Most_Recent_Glycemic_Status_Assessment_Without_Result,
            Has_Most_Recent_Glycemic_Status_Assessment_Without_Result_Compute);
    ...
    public bool? Has_Most_Recent_Elevated_Glycemic_Status_Assessment(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Most_Recent_Elevated_Glycemic_Status_Assessment,
            Has_Most_Recent_Elevated_Glycemic_Status_Assessment_Compute);
    ...
    public bool? Has_No_Record_Of_Glycemic_Status_Assessment(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_No_Record_Of_Glycemic_Status_Assessment,
            Has_No_Record_Of_Glycemic_Status_Assessment_Compute);
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Glycemic_Status_Assessment != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Glycemic_Status_Assessment' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Glycemic_Status_Assessment}}. Cache indices can only be initialized once.");
        _cacheIndex_Glycemic_Status_Assessment = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Most_Recent_Glycemic_Status_Date != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Most_Recent_Glycemic_Status_Date' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Most_Recent_Glycemic_Status_Date}}. Cache indices can only be initialized once.");
        _cacheIndex_Most_Recent_Glycemic_Status_Date = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Lowest_Glycemic_Status_Assessment_Reading_on_Most_Recent_Day != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Lowest_Glycemic_Status_Assessment_Reading_on_Most_Recent_Day' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Lowest_Glycemic_Status_Assessment_Reading_on_Most_Recent_Day}}. Cache indices can only be initialized once.");
        _cacheIndex_Lowest_Glycemic_Status_Assessment_Reading_on_Most_Recent_Day = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Most_Recent_Glycemic_Status_Assessment_Without_Result != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Most_Recent_Glycemic_Status_Assessment_Without_Result' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Most_Recent_Glycemic_Status_Assessment_Without_Result}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Most_Recent_Glycemic_Status_Assessment_Without_Result = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Most_Recent_Elevated_Glycemic_Status_Assessment != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Most_Recent_Elevated_Glycemic_Status_Assessment' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Most_Recent_Elevated_Glycemic_Status_Assessment}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Most_Recent_Elevated_Glycemic_Status_Assessment = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_No_Record_Of_Glycemic_Status_Assessment != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_No_Record_Of_Glycemic_Status_Assessment' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_No_Record_Of_Glycemic_Status_Assessment}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_No_Record_Of_Glycemic_Status_Assessment = initializer.GetNextIndex();
        count++;
