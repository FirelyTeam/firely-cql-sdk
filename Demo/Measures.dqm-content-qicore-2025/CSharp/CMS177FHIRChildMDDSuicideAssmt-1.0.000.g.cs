...
public partial class CMS177FHIRChildMDDSuicideAssmt_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS177FHIRChildMDDSuicideAssmt_1_0_000>
{
    #region ValueSets (9)
    ...
    public IEnumerable<Encounter> Encounter_With_Condition_Major_Depressive_Disorder(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Encounter_With_Condition_Major_Depressive_Disorder,
            Encounter_With_Condition_Major_Depressive_Disorder_Compute);
    ...
    public IEnumerable<Encounter> Encounter_With_Reason_Major_Depressive_Disorder(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Encounter_With_Reason_Major_Depressive_Disorder,
            Encounter_With_Reason_Major_Depressive_Disorder_Compute);
    ...
    public IEnumerable<Encounter> Major_Depressive_Disorder_Encounter(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Major_Depressive_Disorder_Encounter,
            Major_Depressive_Disorder_Encounter_Compute);
    ...
    public IEnumerable<Encounter> Encounter_With_Procedure_For_Suicide_Risk_Assessment(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Encounter_With_Procedure_For_Suicide_Risk_Assessment,
            Encounter_With_Procedure_For_Suicide_Risk_Assessment_Compute);
    ...
    public IEnumerable<Encounter> Encounter_With_Observation_For_Suicide_Risk_Assessment(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Encounter_With_Observation_For_Suicide_Risk_Assessment,
            Encounter_With_Observation_For_Suicide_Risk_Assessment_Compute);
    ...
    private int _cacheIndex_Encounter_With_Condition_Major_Depressive_Disorder = -1;
    private int _cacheIndex_Encounter_With_Reason_Major_Depressive_Disorder = -1;
    private int _cacheIndex_Major_Depressive_Disorder_Encounter = -1;
    private int _cacheIndex_Initial_Population = -1;
    private int _cacheIndex_Denominator = -1;
    private int _cacheIndex_Encounter_With_Procedure_For_Suicide_Risk_Assessment = -1;
    private int _cacheIndex_Encounter_With_Observation_For_Suicide_Risk_Assessment = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Encounter_With_Condition_Major_Depressive_Disorder != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Encounter_With_Condition_Major_Depressive_Disorder' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Encounter_With_Condition_Major_Depressive_Disorder}}. Cache indices can only be initialized once.");
        _cacheIndex_Encounter_With_Condition_Major_Depressive_Disorder = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Encounter_With_Reason_Major_Depressive_Disorder != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Encounter_With_Reason_Major_Depressive_Disorder' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Encounter_With_Reason_Major_Depressive_Disorder}}. Cache indices can only be initialized once.");
        _cacheIndex_Encounter_With_Reason_Major_Depressive_Disorder = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Major_Depressive_Disorder_Encounter != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Major_Depressive_Disorder_Encounter' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Major_Depressive_Disorder_Encounter}}. Cache indices can only be initialized once.");
        _cacheIndex_Major_Depressive_Disorder_Encounter = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Encounter_With_Procedure_For_Suicide_Risk_Assessment != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Encounter_With_Procedure_For_Suicide_Risk_Assessment' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Encounter_With_Procedure_For_Suicide_Risk_Assessment}}. Cache indices can only be initialized once.");
        _cacheIndex_Encounter_With_Procedure_For_Suicide_Risk_Assessment = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Encounter_With_Observation_For_Suicide_Risk_Assessment != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Encounter_With_Observation_For_Suicide_Risk_Assessment' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Encounter_With_Observation_For_Suicide_Risk_Assessment}}. Cache indices can only be initialized once.");
        _cacheIndex_Encounter_With_Observation_For_Suicide_Risk_Assessment = initializer.GetNextIndex();
        count++;
