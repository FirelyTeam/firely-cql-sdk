...
public partial class CMS1056FHIRCTClinical_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS1056FHIRCTClinical_1_0_000>
{
    ...
    public IEnumerable<Observation> Patients_with_Qualifying_CTScan(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Observation>>(
            _cacheIndex_Patients_with_Qualifying_CTScan,
            Patients_with_Qualifying_CTScan_Compute);
    ...
    public IEnumerable<Observation> Patients_with_Qualifying_CTScan_with_Values(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Observation>>(
            _cacheIndex_Patients_with_Qualifying_CTScan_with_Values,
            Patients_with_Qualifying_CTScan_with_Values_Compute);
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Patients_with_Qualifying_CTScan != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Patients_with_Qualifying_CTScan' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Patients_with_Qualifying_CTScan}}. Cache indices can only be initialized once.");
        _cacheIndex_Patients_with_Qualifying_CTScan = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Patients_with_Qualifying_CTScan_with_Values != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Patients_with_Qualifying_CTScan_with_Values' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Patients_with_Qualifying_CTScan_with_Values}}. Cache indices can only be initialized once.");
        _cacheIndex_Patients_with_Qualifying_CTScan_with_Values = initializer.GetNextIndex();
        count++;
