...
public partial class CMS646FHIRIntravesicalBCGTherapy_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS646FHIRIntravesicalBCGTherapy_1_0_000>
{
        ...
        int startIndex)
    {
        ...
        _cacheIndex_Bladder_Cancer_Diagnosis = index++;
        _cacheIndex_First_Bladder_Cancer_Staging_Procedure = index++;
        _cacheIndex_July_1_of_Year_Prior_to_the_Measurement_Period = index++;
        _cacheIndex_June_30_of_the_Measurement_Period = index++;
        _cacheIndex_First_Bladder_Cancer_Staging_Procedure_during_6_Months_Prior_to_Measurement_Period_through_the_First_6_Months_of_Measurement_Period = index++;
        _cacheIndex_Has_Most_Recent_Bladder_Cancer_Tumor_Staging_is_T1__Tis__Ta_HG = index++;
        _cacheIndex_Has_Qualifying_Encounter = index++;
        _cacheIndex_Initial_Population = index++;
        _cacheIndex_Denominator = index++;
        _cacheIndex_Acute_Tuberculosis_Diagnosis = index++;
        _cacheIndex_Immunosuppressive_Drugs = index++;
        _cacheIndex_Cystectomy_Done = index++;
        _cacheIndex_Has_Excluding_HIV__Immunocompromised_Conditions_or_Mixed_Histology_Before_Staging = index++;
        _cacheIndex_Has_Excluding_Chemotherapy_or_Radiotherapy_Procedure_Before_Staging = index++;
        _cacheIndex_Denominator_Exclusion = index++;
        _cacheIndex_BCG_Not_Available_Within_6_Months_After_Bladder_Cancer_Staging = index++;
        _cacheIndex_Denominator_Exception = index++;
        _cacheIndex_First_BCG_Administered = index++;
