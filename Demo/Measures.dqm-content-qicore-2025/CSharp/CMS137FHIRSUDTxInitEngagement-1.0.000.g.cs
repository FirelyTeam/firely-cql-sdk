...
public partial class CMS137FHIRSUDTxInitEngagement_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS137FHIRSUDTxInitEngagement_1_0_000>
{
    #region ValueSets (15)
    ...
    public Encounter First_SUD_Episode_During_Measurement_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<Encounter>(
            _cacheIndex_First_SUD_Episode_During_Measurement_Period,
            First_SUD_Episode_During_Measurement_Period_Compute);
    ...
    public IEnumerable<Encounter> Emergency_Department_or_Detoxification_Visit(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Emergency_Department_or_Detoxification_Visit,
            Emergency_Department_or_Detoxification_Visit_Compute);
    ...
    public IEnumerable<object> History_of_SUD_Diagnosis_or_Treatment(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<object>>(
            _cacheIndex_History_of_SUD_Diagnosis_or_Treatment,
            History_of_SUD_Diagnosis_or_Treatment_Compute);
    ...
    public IEnumerable<object> Psychosocial_Visit(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<object>>(
            _cacheIndex_Psychosocial_Visit,
            Psychosocial_Visit_Compute);
    ...
    public IEnumerable<CqlDate> Treatment_Initiation_With_Non_Medication_Intervention_Dates(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<CqlDate>>(
            _cacheIndex_Treatment_Initiation_With_Non_Medication_Intervention_Dates,
            Treatment_Initiation_With_Non_Medication_Intervention_Dates_Compute);
    ...
    public IEnumerable<CqlDate> Treatment_Initiation_With_Medication_Order_Dates(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<CqlDate>>(
            _cacheIndex_Treatment_Initiation_With_Medication_Order_Dates,
            Treatment_Initiation_With_Medication_Order_Dates_Compute);
    ...
    public bool? Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention,
            Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention_Compute);
    ...
    public bool? Has_Treatment_Engagement_With_Long_Acting_Medication(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Treatment_Engagement_With_Long_Acting_Medication,
            Has_Treatment_Engagement_With_Long_Acting_Medication_Compute);
    ...
    private int _cacheIndex_First_SUD_Episode_During_Measurement_Period = -1;
    private int _cacheIndex_Emergency_Department_or_Detoxification_Visit = -1;
    private int _cacheIndex_History_of_SUD_Diagnosis_or_Treatment = -1;
    ...
    private int _cacheIndex_Psychosocial_Visit = -1;
    private int _cacheIndex_Treatment_Initiation_With_Non_Medication_Intervention_Dates = -1;
    private int _cacheIndex_Treatment_Initiation_With_Medication_Order_Dates = -1;
    private int _cacheIndex_Numerator_1 = -1;
    private int _cacheIndex_Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention = -1;
    private int _cacheIndex_Has_Treatment_Engagement_With_Long_Acting_Medication = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_First_SUD_Episode_During_Measurement_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_First_SUD_Episode_During_Measurement_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_First_SUD_Episode_During_Measurement_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_First_SUD_Episode_During_Measurement_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Emergency_Department_or_Detoxification_Visit != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Emergency_Department_or_Detoxification_Visit' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Emergency_Department_or_Detoxification_Visit}}. Cache indices can only be initialized once.");
        _cacheIndex_Emergency_Department_or_Detoxification_Visit = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_History_of_SUD_Diagnosis_or_Treatment != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_History_of_SUD_Diagnosis_or_Treatment' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_History_of_SUD_Diagnosis_or_Treatment}}. Cache indices can only be initialized once.");
        _cacheIndex_History_of_SUD_Diagnosis_or_Treatment = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Psychosocial_Visit != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Psychosocial_Visit' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Psychosocial_Visit}}. Cache indices can only be initialized once.");
        _cacheIndex_Psychosocial_Visit = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Treatment_Initiation_With_Non_Medication_Intervention_Dates != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Treatment_Initiation_With_Non_Medication_Intervention_Dates' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Treatment_Initiation_With_Non_Medication_Intervention_Dates}}. Cache indices can only be initialized once.");
        _cacheIndex_Treatment_Initiation_With_Non_Medication_Intervention_Dates = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Treatment_Initiation_With_Medication_Order_Dates != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Treatment_Initiation_With_Medication_Order_Dates' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Treatment_Initiation_With_Medication_Order_Dates}}. Cache indices can only be initialized once.");
        _cacheIndex_Treatment_Initiation_With_Medication_Order_Dates = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Two_or_More_Treatment_Engagements_With_Short_Acting_Medication_or_Non_Medication_Intervention = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Treatment_Engagement_With_Long_Acting_Medication != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Treatment_Engagement_With_Long_Acting_Medication' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Treatment_Engagement_With_Long_Acting_Medication}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Treatment_Engagement_With_Long_Acting_Medication = initializer.GetNextIndex();
        count++;
