...
public partial class CMS144FHIRHFBetaBlockerForLVSD_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS144FHIRHFBetaBlockerForLVSD_1_0_000>
{
    #region ValueSets (13)
    ...
    public bool? Has_Consecutive_Heart_Rates_Less_than_50(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Consecutive_Heart_Rates_Less_than_50,
            Has_Consecutive_Heart_Rates_Less_than_50_Compute);
    ...
    public bool? Has_Arrhythmia_Diagnosis(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Arrhythmia_Diagnosis,
            Has_Arrhythmia_Diagnosis_Compute);
    ...
    public bool? Has_Hypotension_Diagnosis(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Hypotension_Diagnosis,
            Has_Hypotension_Diagnosis_Compute);
    ...
    public bool? Has_Asthma_Diagnosis(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Asthma_Diagnosis,
            Has_Asthma_Diagnosis_Compute);
    ...
    public bool? Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy,
            Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Compute);
    ...
    public bool? Has_Bradycardia_Diagnosis(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Bradycardia_Diagnosis,
            Has_Bradycardia_Diagnosis_Compute);
    ...
    public bool? Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient,
            Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient_Compute);
    ...
    public bool? Has_Atrioventricular_Block_Diagnosis(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Atrioventricular_Block_Diagnosis,
            Has_Atrioventricular_Block_Diagnosis_Compute);
    ...
    public bool? Has_Diagnosis_of_Cardiac_Pacer_in_Situ(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Diagnosis_of_Cardiac_Pacer_in_Situ,
            Has_Diagnosis_of_Cardiac_Pacer_in_Situ_Compute);
    ...
    public bool? Has_Cardiac_Pacer_Device_Implanted(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Cardiac_Pacer_Device_Implanted,
            Has_Cardiac_Pacer_Device_Implanted_Compute);
    ...
    public bool? Atrioventricular_Block_without_Cardiac_Pacer(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Atrioventricular_Block_without_Cardiac_Pacer,
            Atrioventricular_Block_without_Cardiac_Pacer_Compute);
    ...
    private int _cacheIndex_Has_Consecutive_Heart_Rates_Less_than_50 = -1;
    private int _cacheIndex_Has_Medical_or_Patient_Reason_for_Not_Ordering_Beta_Blocker_for_LVSD = -1;
    private int _cacheIndex_Has_Arrhythmia_Diagnosis = -1;
    private int _cacheIndex_Has_Hypotension_Diagnosis = -1;
    private int _cacheIndex_Has_Asthma_Diagnosis = -1;
    private int _cacheIndex_Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy = -1;
    private int _cacheIndex_Has_Bradycardia_Diagnosis = -1;
    private int _cacheIndex_Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient = -1;
    private int _cacheIndex_Has_Atrioventricular_Block_Diagnosis = -1;
    private int _cacheIndex_Has_Diagnosis_of_Cardiac_Pacer_in_Situ = -1;
    private int _cacheIndex_Has_Cardiac_Pacer_Device_Implanted = -1;
    private int _cacheIndex_Atrioventricular_Block_without_Cardiac_Pacer = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Has_Consecutive_Heart_Rates_Less_than_50 != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Consecutive_Heart_Rates_Less_than_50' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Consecutive_Heart_Rates_Less_than_50}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Consecutive_Heart_Rates_Less_than_50 = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Has_Arrhythmia_Diagnosis != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Arrhythmia_Diagnosis' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Arrhythmia_Diagnosis}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Arrhythmia_Diagnosis = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Hypotension_Diagnosis != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Hypotension_Diagnosis' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Hypotension_Diagnosis}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Hypotension_Diagnosis = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Asthma_Diagnosis != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Asthma_Diagnosis' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Asthma_Diagnosis}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Asthma_Diagnosis = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Diagnosis_of_Allergy_or_Intolerance_to_Beta_Blocker_Therapy = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Bradycardia_Diagnosis != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Bradycardia_Diagnosis' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Bradycardia_Diagnosis}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Bradycardia_Diagnosis = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Allergy_or_Intolerance_to_Beta_Blocker_Therapy_Ingredient = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Atrioventricular_Block_Diagnosis != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Atrioventricular_Block_Diagnosis' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Atrioventricular_Block_Diagnosis}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Atrioventricular_Block_Diagnosis = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Diagnosis_of_Cardiac_Pacer_in_Situ != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Diagnosis_of_Cardiac_Pacer_in_Situ' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Diagnosis_of_Cardiac_Pacer_in_Situ}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Diagnosis_of_Cardiac_Pacer_in_Situ = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Cardiac_Pacer_Device_Implanted != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Cardiac_Pacer_Device_Implanted' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Cardiac_Pacer_Device_Implanted}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Cardiac_Pacer_Device_Implanted = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Atrioventricular_Block_without_Cardiac_Pacer != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Atrioventricular_Block_without_Cardiac_Pacer' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Atrioventricular_Block_without_Cardiac_Pacer}}. Cache indices can only be initialized once.");
        _cacheIndex_Atrioventricular_Block_without_Cardiac_Pacer = initializer.GetNextIndex();
        count++;
