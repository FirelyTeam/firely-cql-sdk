...
public partial class CMS1074FHIRCTIQR_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS1074FHIRCTIQR_1_0_000>
{
    ...
    public IEnumerable<Encounter> Qualifying_Inpatient_Encounters(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Qualifying_Inpatient_Encounters,
            Qualifying_Inpatient_Encounters_Compute);
    ...
    public IEnumerable<Observation> Qualifying_Scan_During_Inpatient_Encounter(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Observation>>(
            _cacheIndex_Qualifying_Scan_During_Inpatient_Encounter,
            Qualifying_Scan_During_Inpatient_Encounter_Compute);
    ...
    public IEnumerable<Observation> Qualifying_Scan_With_Values(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Observation>>(
            _cacheIndex_Qualifying_Scan_With_Values,
            Qualifying_Scan_With_Values_Compute);
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Qualifying_Inpatient_Encounters != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Qualifying_Inpatient_Encounters' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Qualifying_Inpatient_Encounters}}. Cache indices can only be initialized once.");
        _cacheIndex_Qualifying_Inpatient_Encounters = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Qualifying_Scan_During_Inpatient_Encounter != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Qualifying_Scan_During_Inpatient_Encounter' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Qualifying_Scan_During_Inpatient_Encounter}}. Cache indices can only be initialized once.");
        _cacheIndex_Qualifying_Scan_During_Inpatient_Encounter = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Qualifying_Scan_With_Values != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Qualifying_Scan_With_Values' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Qualifying_Scan_With_Values}}. Cache indices can only be initialized once.");
        _cacheIndex_Qualifying_Scan_With_Values = initializer.GetNextIndex();
        count++;
