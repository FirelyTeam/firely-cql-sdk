...
public partial class CMS159FHIRDepRemissionat12Months_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS159FHIRDepRemissionat12Months_1_0_000>
{
    #region ValueSets (16)
    ...
    public IEnumerable<Observation> Depression_Assessments_Greater_Than_9(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Observation>>(
            _cacheIndex_Depression_Assessments_Greater_Than_9,
            Depression_Assessments_Greater_Than_9_Compute);
    ...
    public IEnumerable<object> Depression_Diagnoses(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<object>>(
            _cacheIndex_Depression_Diagnoses,
            Depression_Diagnoses_Compute);
    ...
    public CqlInterval<CqlDateTime> Denominator_Identification_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<CqlInterval<CqlDateTime>>(
            _cacheIndex_Denominator_Identification_Period,
            Denominator_Identification_Period_Compute);
    ...
    public IEnumerable<Encounter> Depression_Encounter(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Depression_Encounter,
            Depression_Encounter_Compute);
    ...
    public Observation Index_Depression_Assessment(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<Observation>(
            _cacheIndex_Index_Depression_Assessment,
            Index_Depression_Assessment_Compute);
    ...
    public CqlInterval<CqlDate> Measure_Assessment_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<CqlInterval<CqlDate>>(
            _cacheIndex_Measure_Assessment_Period,
            Measure_Assessment_Period_Compute);
    ...
    public bool? Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period,
            Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period_Compute);
    ...
    public bool? Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period,
            Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period_Compute);
    ...
    public bool? Patient_Expired(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Patient_Expired,
            Patient_Expired_Compute);
    ...
    public bool? Has_Mental_Health_Disorder_Diagnoses(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Mental_Health_Disorder_Diagnoses,
            Has_Mental_Health_Disorder_Diagnoses_Compute);
    ...
    private int _cacheIndex_Depression_Assessments_Greater_Than_9 = -1;
    private int _cacheIndex_Depression_Diagnoses = -1;
    private int _cacheIndex_Denominator_Identification_Period = -1;
    private int _cacheIndex_Depression_Encounter = -1;
    private int _cacheIndex_Index_Depression_Assessment = -1;
    private int _cacheIndex_Initial_Population = -1;
    private int _cacheIndex_Denominator = -1;
    private int _cacheIndex_Measure_Assessment_Period = -1;
    private int _cacheIndex_Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period = -1;
    private int _cacheIndex_Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period = -1;
    private int _cacheIndex_Patient_Expired = -1;
    private int _cacheIndex_Has_Mental_Health_Disorder_Diagnoses = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Depression_Assessments_Greater_Than_9 != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Depression_Assessments_Greater_Than_9' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Depression_Assessments_Greater_Than_9}}. Cache indices can only be initialized once.");
        _cacheIndex_Depression_Assessments_Greater_Than_9 = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Depression_Diagnoses != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Depression_Diagnoses' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Depression_Diagnoses}}. Cache indices can only be initialized once.");
        _cacheIndex_Depression_Diagnoses = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Denominator_Identification_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Denominator_Identification_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Denominator_Identification_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Denominator_Identification_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Depression_Encounter != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Depression_Encounter' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Depression_Encounter}}. Cache indices can only be initialized once.");
        _cacheIndex_Depression_Encounter = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Index_Depression_Assessment != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Index_Depression_Assessment' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Index_Depression_Assessment}}. Cache indices can only be initialized once.");
        _cacheIndex_Index_Depression_Assessment = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Measure_Assessment_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Measure_Assessment_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Measure_Assessment_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Measure_Assessment_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Hospice_Services_prior_to_or_during_the_Measure_Assessment_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Palliative_Care_prior_to_or_during_the_Measure_Assessment_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Patient_Expired != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Patient_Expired' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Patient_Expired}}. Cache indices can only be initialized once.");
        _cacheIndex_Patient_Expired = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Mental_Health_Disorder_Diagnoses != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Mental_Health_Disorder_Diagnoses' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Mental_Health_Disorder_Diagnoses}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Mental_Health_Disorder_Diagnoses = initializer.GetNextIndex();
        count++;
