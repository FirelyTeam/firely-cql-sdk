...
public partial class CMS154FHIRAppropriateTxforURI_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS154FHIRAppropriateTxforURI_1_0_000>
{
    ...
    public IEnumerable<Encounter> Encounter_with_Upper_Respiratory_Infection(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Encounter>>(
            _cacheIndex_Encounter_with_Upper_Respiratory_Infection,
            Encounter_with_Upper_Respiratory_Infection_Compute);
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Encounter_with_Upper_Respiratory_Infection != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Encounter_with_Upper_Respiratory_Infection' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Encounter_with_Upper_Respiratory_Infection}}. Cache indices can only be initialized once.");
        _cacheIndex_Encounter_with_Upper_Respiratory_Infection = initializer.GetNextIndex();
        count++;
