...
public partial class CMS129FHIRProstCaBoneScanUse_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS129FHIRProstCaBoneScanUse_1_0_000>
{
    #region ValueSets (6)
    ...
    public Procedure First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<Procedure>(
            _cacheIndex_First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period,
            First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period_Compute);
    ...
    public Observation Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<Observation>(
            _cacheIndex_Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a,
            Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a_Compute);
    ...
    public bool? Most_Recent_PSA_Test_Result_is_Low(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Most_Recent_PSA_Test_Result_is_Low,
            Most_Recent_PSA_Test_Result_is_Low_Compute);
    ...
    public bool? Most_Recent_Gleason_Score_is_Low(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Most_Recent_Gleason_Score_is_Low,
            Most_Recent_Gleason_Score_is_Low_Compute);
    ...
    public IEnumerable<Observation> Bone_Scan_Study_Performed(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<Observation>>(
            _cacheIndex_Bone_Scan_Study_Performed,
            Bone_Scan_Study_Performed_Compute);
    ...
    public bool? Has_Diagnosis_of_Pain_related_to_Prostate_Cancer(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Diagnosis_of_Pain_related_to_Prostate_Cancer,
            Has_Diagnosis_of_Pain_related_to_Prostate_Cancer_Compute);
    ...
    public bool? Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis,
            Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis_Compute);
    ...
    public bool? Has_Bone_Scan_Study_Performed_with_Documented_Reason(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Bone_Scan_Study_Performed_with_Documented_Reason,
            Has_Bone_Scan_Study_Performed_with_Documented_Reason_Compute);
    ...
    private int _cacheIndex_Prostate_Cancer_Diagnosis = -1;
    private int _cacheIndex_Initial_Population = -1;
    private int _cacheIndex_First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period = -1;
    private int _cacheIndex_Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a = -1;
    private int _cacheIndex_Most_Recent_PSA_Test_Result_is_Low = -1;
    private int _cacheIndex_Most_Recent_Gleason_Score_is_Low = -1;
    private int _cacheIndex_Denominator = -1;
    private int _cacheIndex_Bone_Scan_Study_Performed = -1;
    private int _cacheIndex_Numerator = -1;
    private int _cacheIndex_Has_Diagnosis_of_Pain_related_to_Prostate_Cancer = -1;
    private int _cacheIndex_Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis = -1;
    private int _cacheIndex_Has_Bone_Scan_Study_Performed_with_Documented_Reason = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_First_Prostate_Cancer_Treatment_during_day_of_Measurement_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a}}. Cache indices can only be initialized once.");
        _cacheIndex_Most_Recent_Prostate_Cancer_Staging_Tumor_Size_T1a_to_T2a = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Most_Recent_PSA_Test_Result_is_Low != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Most_Recent_PSA_Test_Result_is_Low' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Most_Recent_PSA_Test_Result_is_Low}}. Cache indices can only be initialized once.");
        _cacheIndex_Most_Recent_PSA_Test_Result_is_Low = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Most_Recent_Gleason_Score_is_Low != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Most_Recent_Gleason_Score_is_Low' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Most_Recent_Gleason_Score_is_Low}}. Cache indices can only be initialized once.");
        _cacheIndex_Most_Recent_Gleason_Score_is_Low = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Bone_Scan_Study_Performed != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Bone_Scan_Study_Performed' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Bone_Scan_Study_Performed}}. Cache indices can only be initialized once.");
        _cacheIndex_Bone_Scan_Study_Performed = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Has_Diagnosis_of_Pain_related_to_Prostate_Cancer != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Diagnosis_of_Pain_related_to_Prostate_Cancer' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Diagnosis_of_Pain_related_to_Prostate_Cancer}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Diagnosis_of_Pain_related_to_Prostate_Cancer = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Salvage_Therapy_Performed_after_Prostate_Cancer_Diagnosis = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Bone_Scan_Study_Performed_with_Documented_Reason != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Bone_Scan_Study_Performed_with_Documented_Reason' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Bone_Scan_Study_Performed_with_Documented_Reason}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Bone_Scan_Study_Performed_with_Documented_Reason = initializer.GetNextIndex();
        count++;
