...
public partial class CMS314FHIRHIVViralSuppression_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS314FHIRHIVViralSuppression_1_0_000>
{
    #region ValueSets (13)
    ...
    public bool? Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period,
            Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period_Compute);
    ...
    public Observation Most_Recent_Viral_Load_Test_During_Measurement_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<Observation>(
            _cacheIndex_Most_Recent_Viral_Load_Test_During_Measurement_Period,
            Most_Recent_Viral_Load_Test_During_Measurement_Period_Compute);
    ...
    private int _cacheIndex_Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period = -1;
    private int _cacheIndex_Has_Qualifying_Encounter_During_First_240_Days_of_Measurement_Period = -1;
    private int _cacheIndex_Initial_Population = -1;
    private int _cacheIndex_Denominator = -1;
    private int _cacheIndex_Most_Recent_Viral_Load_Test_During_Measurement_Period = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Active_HIV_Diagnosis_Before_or_in_First_90_Days_of_Measurement_Period = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Most_Recent_Viral_Load_Test_During_Measurement_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Most_Recent_Viral_Load_Test_During_Measurement_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Most_Recent_Viral_Load_Test_During_Measurement_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Most_Recent_Viral_Load_Test_During_Measurement_Period = initializer.GetNextIndex();
        count++;
