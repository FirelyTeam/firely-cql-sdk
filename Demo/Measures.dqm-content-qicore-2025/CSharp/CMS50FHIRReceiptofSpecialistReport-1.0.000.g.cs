...
public partial class CMS50FHIRReceiptofSpecialistReport_1_0_000 : ILibrary, ILibraryInternals, ISingleton<CMS50FHIRReceiptofSpecialistReport_1_0_000>
{
    #region ValueSets (10)
    ...
    public bool? Has_Encounter_during_Measurement_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Encounter_during_Measurement_Period,
            Has_Encounter_during_Measurement_Period_Compute);
    ...
    public bool? Has_Intervention_during_Measurement_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Has_Intervention_during_Measurement_Period,
            Has_Intervention_during_Measurement_Period_Compute);
    ...
    public ServiceRequest First_Referral_during_First_10_Months_of_Measurement_Period(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<ServiceRequest>(
            _cacheIndex_First_Referral_during_First_10_Months_of_Measurement_Period,
            First_Referral_during_First_10_Months_of_Measurement_Period_Compute);
    ...
    public bool? Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop,
            Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop_Compute);
    ...
    private int _cacheIndex_Has_Encounter_during_Measurement_Period = -1;
    private int _cacheIndex_Has_Intervention_during_Measurement_Period = -1;
    private int _cacheIndex_First_Referral_during_First_10_Months_of_Measurement_Period = -1;
    ...
    private int _cacheIndex_Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Has_Encounter_during_Measurement_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Encounter_during_Measurement_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Encounter_during_Measurement_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Encounter_during_Measurement_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Has_Intervention_during_Measurement_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Has_Intervention_during_Measurement_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Has_Intervention_during_Measurement_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_Has_Intervention_during_Measurement_Period = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_First_Referral_during_First_10_Months_of_Measurement_Period != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_First_Referral_during_First_10_Months_of_Measurement_Period' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_First_Referral_during_First_10_Months_of_Measurement_Period}}. Cache indices can only be initialized once.");
        _cacheIndex_First_Referral_during_First_10_Months_of_Measurement_Period = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop}}. Cache indices can only be initialized once.");
        _cacheIndex_Referring_Clinician_Receives_Consultant_Report_to_Close_Referral_Loop = initializer.GetNextIndex();
        count++;
