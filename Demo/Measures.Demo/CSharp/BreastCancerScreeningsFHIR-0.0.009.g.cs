...
public partial class BreastCancerScreeningsFHIR_0_0_009 : ILibrary, ILibraryInternals, ISingleton<BreastCancerScreeningsFHIR_0_0_009>
{
    #region ValueSets (12)
    ...
    public bool? Observation_with_status(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Observation_with_status,
            Observation_with_status_Compute);
    ...
    public bool? Diagnostic_Report_with_status(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Diagnostic_Report_with_status,
            Diagnostic_Report_with_status_Compute);
    ...
    public bool? Observation_without_appropriate_status(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Observation_without_appropriate_status,
            Observation_without_appropriate_status_Compute);
    ...
    public bool? Diagnostic_Report_without_appropriate_status(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Diagnostic_Report_without_appropriate_status,
            Diagnostic_Report_without_appropriate_status_Compute);
    ...
    private int _cacheIndex_Telehealth_Services = -1;
    private int _cacheIndex_Age_at_start_of_Measurement_Period = -1;
    ...
    private int _cacheIndex_Observation_with_status = -1;
    private int _cacheIndex_Diagnostic_Report_with_status = -1;
    private int _cacheIndex_Numerator = -1;
    private int _cacheIndex_Final_Numerator_Population = -1;
    private int _cacheIndex_Observation_without_appropriate_status = -1;
    private int _cacheIndex_Diagnostic_Report_without_appropriate_status = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Observation_with_status != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Observation_with_status' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Observation_with_status}}. Cache indices can only be initialized once.");
        _cacheIndex_Observation_with_status = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Diagnostic_Report_with_status != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Diagnostic_Report_with_status' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Diagnostic_Report_with_status}}. Cache indices can only be initialized once.");
        _cacheIndex_Diagnostic_Report_with_status = initializer.GetNextIndex();
        count++;
        ...
        if (_cacheIndex_Observation_without_appropriate_status != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Observation_without_appropriate_status' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Observation_without_appropriate_status}}. Cache indices can only be initialized once.");
        _cacheIndex_Observation_without_appropriate_status = initializer.GetNextIndex();
        count++;

        if (_cacheIndex_Diagnostic_Report_without_appropriate_status != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Diagnostic_Report_without_appropriate_status' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Diagnostic_Report_without_appropriate_status}}. Cache indices can only be initialized once.");
        _cacheIndex_Diagnostic_Report_without_appropriate_status = initializer.GetNextIndex();
        count++;
