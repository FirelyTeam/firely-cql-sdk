...
public partial class HybridHWMFHIR_0_102_005 : ILibrary, ILibraryInternals, ISingleton<HybridHWMFHIR_0_102_005>
{
    #region ValueSets (15)
    ...
    public IEnumerable<string> Results(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<IEnumerable<string>>(
            _cacheIndex_Results,
            Results_Compute);
    ...
    private int _cacheIndex_Results = -1;
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Results != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Results' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Results}}. Cache indices can only be initialized once.");
        _cacheIndex_Results = initializer.GetNextIndex();
        count++;
