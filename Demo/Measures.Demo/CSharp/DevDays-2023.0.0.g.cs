...
public partial class DevDays_2023_0_0 : ILibrary, ISingleton<DevDays_2023_0_0>
{
    ...
    private const long _cacheIndex_Measurement_Period = 7096341437342020688L;
    ...
    private const long _cacheIndex_Patient = 9124584184355502557L;
    ...
    private const long _cacheIndex_Jet_engine_conditions = 2800937253784641656L;
    ...
    private IEnumerable<Condition> Jet_engine_conditions_Compute(CqlContext context)
    {
        ...
        bool? b_(Condition c) {
            ...
            bool? f_(Coding coding) {
                CqlCode h_ = FHIRHelpers_4_0_001.Instance.ToCode(context, coding);
                CqlCode i_ = this.Sucked_into_jet_engine(context);
                bool? j_ = context.Operators.Equivalent(h_, i_);
                return j_;
            }

            bool? g_ = context.Operators.WhereAny<Coding>((IEnumerable<Coding>)e_, f_);
            // CQL 'and' (18:3-20:57): right operand skipped when left is false
            if (g_ is false)
            {
                return false;
            }
            ...
            {
                DataType k_ = c?.Onset;
                CqlDateTime l_ = FHIRHelpers_4_0_001.Instance.ToDateTime(context, k_ as FhirDateTime);
    ...
    private const long _cacheIndex_Subsequent_encounters = 33888561033543232L;
    ...
    private IEnumerable<Condition> Subsequent_encounters_Compute(CqlContext context)
    {
        ...
        bool? b_(Condition c) {
            ...
            bool? f_(Coding coding) {
                CqlCode h_ = FHIRHelpers_4_0_001.Instance.ToCode(context, coding);
                CqlCode i_ = this.Sucked_into_jet_engine__subsequent_encounter(context);
                bool? j_ = context.Operators.Equivalent(h_, i_);
                return j_;
            }

            bool? g_ = context.Operators.WhereAny<Coding>((IEnumerable<Coding>)e_, f_);
            // CQL 'and' (24:3-26:57): right operand skipped when left is false
            if (g_ is false)
            {
                return false;
            }
    ...
    private const long _cacheIndex_Initial_population = 6175140352612506819L;
    ...
    private const long _cacheIndex_Numerator = 1694005781377430274L;
