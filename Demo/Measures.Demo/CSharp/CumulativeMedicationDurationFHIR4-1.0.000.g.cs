...
public class CumulativeMedicationDurationFHIR4_1_0_000
{
		...
		CqlInterval<CqlDateTime> b_(MedicationRequest R)
		{
			CqlInterval<CqlDateTime> e_()
			{
				bool f_()
				{
					...
					Period l_ = context.Operators.Convert<Period>(k_);
					...
					Period t_ = context.Operators.Convert<Period>(s_);
				...
				if (f_())
				{
					...
					Period af_ = context.Operators.Convert<Period>(ae_);
					...
					Period at_ = context.Operators.Convert<Period>(as_);
					CqlInterval<CqlDateTime> au_ = FHIRHelpers_4_0_001.ToInterval(at_);
				...
				else
				{
					...
					Period bc_ = context.Operators.Convert<Period>(bb_);
					...
					Period bq_ = context.Operators.Convert<Period>(bp_);
					...
					Range ck_ = context.Operators.Convert<Range>(cj_);
					...
					Quantity cs_ = context.Operators.Convert<Quantity>(cr_);
	...
	public CqlInterval<CqlDateTime> MedicationDispensePeriod(MedicationDispense Dispense)
	{
		...
		CqlInterval<CqlDateTime> b_(MedicationDispense D)
		{
			...
			DataType s_ = r_?.Dose;
			Range t_ = context.Operators.Convert<Range>(s_);
			...
			Quantity ab_ = context.Operators.Convert<Quantity>(aa_);
