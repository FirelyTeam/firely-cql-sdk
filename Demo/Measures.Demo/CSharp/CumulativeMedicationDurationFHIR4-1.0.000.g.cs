...
public partial class CumulativeMedicationDurationFHIR4_1_0_000 : ILibrary, ISingleton<CumulativeMedicationDurationFHIR4_1_0_000>
{
    ...
    public CqlInterval<CqlDateTime> MedicationRequestPeriod(CqlContext context, MedicationRequest Request)
    {
        ...
        CqlInterval<CqlDateTime> b_(MedicationRequest R) {
            ...
            if (o_ ?? false)
            {
                ...
                FhirDateTime u_ = R?.AuthoredOnElement;
                CqlDateTime v_ = FHIRHelpers_4_0_001.Instance.ToDateTime(context, u_);
                CqlInterval<CqlDateTime> w_ = context.Operators.Interval((p_ ?? t_) ?? v_, k_, true, true);
                return w_;
            }
            else
            {
                CqlDateTime x_ = context.Operators.Start(j_);
                MedicationRequest.DispenseRequestComponent y_ = R?.DispenseRequest;
                Period z_ = y_?.ValidityPeriod;
                CqlInterval<CqlDateTime> aa_ = FHIRHelpers_4_0_001.Instance.ToInterval(context, z_);
                CqlDateTime ab_ = context.Operators.Start(aa_);
                FhirDateTime ac_ = R?.AuthoredOnElement;
                CqlDateTime ad_ = FHIRHelpers_4_0_001.Instance.ToDateTime(context, ac_);
                Duration ae_ = y_?.ExpectedSupplyDuration;
                CqlQuantity af_ = FHIRHelpers_4_0_001.Instance.ToQuantity(context, ae_);
                Quantity ag_ = y_?.Quantity;
                CqlQuantity ah_ = FHIRHelpers_4_0_001.Instance.ToQuantity(context, ag_);
                List<Dosage.DoseAndRateComponent> ai_ = f_?.DoseAndRate;
                Dosage.DoseAndRateComponent aj_ = context.Operators.SingletonFrom<Dosage.DoseAndRateComponent>((IEnumerable<Dosage.DoseAndRateComponent>)ai_);
                DataType ak_ = aj_?.Dose;
                CqlInterval<CqlQuantity> al_ = FHIRHelpers_4_0_001.Instance.ToInterval(context, ak_ as Range);
                CqlQuantity am_ = context.Operators.End(al_);
                CqlQuantity an_ = FHIRHelpers_4_0_001.Instance.ToQuantity(context, ak_ as Quantity);
                PositiveInt ao_ = h_?.FrequencyMaxElement;
                Integer ap_ = context.Operators.Convert<Integer>(ao_);
                PositiveInt aq_ = h_?.FrequencyElement;
                Integer ar_ = context.Operators.Convert<Integer>(aq_);
                int? as_ = FHIRHelpers_4_0_001.Instance.ToInteger(context, ap_ ?? ar_);
                FhirDecimal at_ = h_?.PeriodElement;
                decimal? au_ = FHIRHelpers_4_0_001.Instance.ToDecimal(context, at_);
                Code<Timing.UnitsOfTime> av_ = h_?.PeriodUnitElement;
                Timing.UnitsOfTime? aw_ = av_?.Value;
                string ax_ = context.Operators.Convert<string>(aw_);
                decimal? ay_ = this.ToDaily(context, as_, new CqlQuantity(au_, ax_));
                List<Time> az_ = h_?.TimeOfDayElement;
                int? ba_ = context.Operators.Count<Time>((IEnumerable<Time>)az_);
                decimal? bb_ = context.Operators.ConvertIntegerToDecimal(ba_);
                CqlQuantity bc_ = context.Operators.ConvertDecimalToQuantity((ay_ ?? bb_) ?? 1.0m);
                CqlQuantity bd_ = context.Operators.Multiply(am_ ?? an_, bc_);
                CqlQuantity be_ = context.Operators.Divide(ah_, bd_);
                UnsignedInt bf_ = y_?.NumberOfRepeatsAllowedElement;
                Integer bg_ = context.Operators.Convert<Integer>(bf_);
                int? bh_ = FHIRHelpers_4_0_001.Instance.ToInteger(context, bg_);
                int? bi_ = context.Operators.Add(1, bh_ ?? 0);
                CqlQuantity bj_ = context.Operators.ConvertIntegerToQuantity(bi_);
                CqlQuantity bk_ = context.Operators.Multiply(af_ ?? be_, bj_);
                CqlDateTime bl_ = context.Operators.Add((x_ ?? ab_) ?? ad_, bk_);
                CqlInterval<CqlDateTime> bm_ = context.Operators.Interval((x_ ?? ab_) ?? ad_, bl_, true, true);
                return bm_;
            }
        }
    ...
    public CqlInterval<CqlDateTime> MedicationAdministrationPeriod(CqlContext context, MedicationAdministration Administration)
    {
        ...
        CqlInterval<CqlDateTime> b_(MedicationAdministration M) {
            ...
            if (m_ ?? false)
            {
                CqlDateTime n_ = context.Operators.Add(g_, k_);
                CqlInterval<CqlDateTime> o_ = context.Operators.Interval(g_, n_, true, true);
                return o_;
            }
            else
            {
