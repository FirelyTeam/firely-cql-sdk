...
public partial class NCQAHospice_1_0_0 : ILibrary, ILibraryInternals, ISingleton<NCQAHospice_1_0_0>
{
    ...
    public bool? Hospice_Intervention_or_Encounter(CqlContext context) =>
        ((ICqlContextInternals)context).GetOrCompute<bool?>(
            _cacheIndex_Hospice_Intervention_or_Encounter,
            Hospice_Intervention_or_Encounter_Compute);
    ...
    int ILibraryInternals.InitializeCacheIndices(CacheIndexInitializer initializer)
    {
        ...
        if (_cacheIndex_Hospice_Intervention_or_Encounter != -1)
            throw new InvalidOperationException($"Cache index field '_cacheIndex_Hospice_Intervention_or_Encounter' in library '{{Name}}' version '{{Version}}' is already initialized to {{_cacheIndex_Hospice_Intervention_or_Encounter}}. Cache indices can only be initialized once.");
        _cacheIndex_Hospice_Intervention_or_Encounter = initializer.GetNextIndex();
        count++;
